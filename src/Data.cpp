...
Dataset::Dataset(const std::string& file) {
...
void Dataset::printStats() {
    ...
    std::cout << YELLOW << "Regression Summary" RESET << "\n";
    ...
    std::cout << BLUE << std::left << std::setw(W_NAME) << labelName << GREEN;
    std::cout << std::fixed << std::setprecision(4)
